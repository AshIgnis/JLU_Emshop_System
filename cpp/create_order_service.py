...
# Read emshop_native_impl_oop.cpp
with open('emshop_native_impl_oop.cpp', 'r', encoding='utf-8') as f:
    lines = f.readlines()

print(f"Main file has {len(lines)} lines")

# Extract OrderService class (lines 1190-2382, 0-indexed: 1189-2381)
order_service_lines = lines[1189:2383]  # Include line 2382 (};)
print(f"OrderService has {len(order_service_lines)} lines")

# Find public methods by parsing class structure
public_methods = []
private_methods = []
in_public = False
in_private = False
...
for i, line in enumerate(order_service_lines):
    stripped = line.strip()
    if stripped.startswith('public:'):
        ...
        in_public = False
        in_private = True
    elif in_public and 'json ' in line and '(' in line:
        # Extract method signature
        method_line = line.strip()
        if not method_line.startswith('//'):
            public_methods.append((i, method_line))
    elif in_private and ('std::string' in line or 'bool ' in line or 'void ' in line) and '(' in line:
        method_line = line.strip()
        if not method_line.startswith('//'):
            private_methods.append((i, method_line))

print(f"Found {len(public_methods)} public methods")
print(f"Found {len(private_methods)} private helper methods")
...
with open('services/OrderService.h', 'w', encoding='utf-8') as f:
    f.write(header_content)

print("OrderService.h created")
...
# Add the class implementation (everything except 'class OrderService : public BaseService {' and final '};')
# Skip first line (class declaration) and last line (};)
impl_lines = []
for i, line in enumerate(order_service_lines):
    if i == 0:  # Skip class declaration line
        continue
    if i == len(order_service_lines) - 1:  # Skip final };
        continue
    # Keep all other content
    impl_lines.append(line)

# Fix: prepend OrderService:: to method definitions
final_impl = []
for line in impl_lines:
    # Skip lines that are already inside class scope markers
    if line.strip() in ['private:', 'public:', 'protected:']:
        continue
    
    # Add OrderService:: to method implementations
    # Match: "json methodName(" or "std::string methodName(" at start of line (after whitespace)
    if re.match(r'^\s+(json|std::string|bool|void)\s+\w+\s*\(', line):
        # Extract method signature
        match = re.match(r'^(\s+)(json|std::string|bool|void)(\s+)(\w+)(\s*\()', line)
        if match:
            indent, ret_type, space1, method_name, open_paren = match.groups()
            # Don't add OrderService:: to const helper methods that are already scoped
            if 'getOrderIdColumnName' not in method_name and \
               'getOrderNoColumnName' not in method_name and \
               'getOrderCreatedAtColumnName' not in method_name and \
               'getOrderUpdatedAtColumnName' not in method_name and \
               'getUsersPrimaryKeyColumn' not in method_name and \
               'orderHasColumn' not in method_name:
                line = f'{ret_type} OrderService::{method_name}{open_paren}' + line[match.end():]
    
    final_impl.append(line)

impl_content += ''.join(final_impl)

with open('services/OrderService.cpp', 'w', encoding='utf-8') as f:
    f.write(impl_content)

print("OrderService.cpp created")
print(f"Implementation file has {len(final_impl)} lines")
