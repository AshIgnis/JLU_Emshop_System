...
javac -d target\classes -cp "src\main\java;target\classes" src\main\java\emshop\EmshopCSCore.java

REM 检查Netty是否可用
echo Checking for Netty dependencies...
echo Note: Netty-based classes require Maven dependencies to be resolved.
echo Please run 'mvn compile' to compile Netty classes.

echo.
echo Basic compilation completed. The following classes are ready:
echo - EmshopCSCore (Traditional Socket Server/Client)
echo - EmshopServer (Business Logic with Mock Data)
echo - MockDataProvider (Test Data Provider)
echo.
echo To use Netty features, run: mvn compile
echo To run the traditional socket server: java -cp target\classes emshop.EmshopCSCore$Server
